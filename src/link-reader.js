 ...
 * @param {any} params.count The number of links per sitemap.
...
const linkReader = async params => {
  ...
  for (let number = 0; number < quantity; number++) {
    // Get domain and path using the first url if the predefinedDomain variable is absent
    const domain = predefinedDomain
      ? predefinedDomain
      : domainExtractor(urls[0]);
